...
pipeline lấy chính dataset của mini-VLA đã thu thập phía trên 
...
### 10.1 Chạy HSV + Goal V2 từ camera
 ...
 Nội dung nằm trong cript chính:
...
### 10.2 Chạy Goal V2 bằng object feature nhập tay
 ...
 nội dung nằm trong Script chính:
...
Nội dung Script thu dữ liệu nằm trong :
...
Máy ảo được thuê trên : https://www.ezycloudx.com/
Nhóm thuê máy docket GPU 
Cấu hình Docker Container-> Chọn ubuntu22.04-devel

Thành phần	             Cấu hình
GPU	                     Tesla V100-SXM2 32GB VRAM
CPU	                     khoảng 16 vCPU
RAM	                     khoảng 64 GB
...
Sau khi clone TinyVLA, cài đặt các thư viện cần thiết:
...
### 12.2. Chuẩn bị backbone

Tiny-VLA sử dụng backbone Vision-Language Model (VLM) Llava-Pythia-400M.

Download backbone từ HuggingFace trước khi train:

```bash
huggingface-cli download \
    VLA-Project/Llava-Pythia-400M \
    --local-dir $TINYVLA_MODEL_BASE
```

hoặc tải thủ công và đặt vào thư mục:
...
### 12.3. Huấn luyện Tiny-VLA

Quá trình huấn luyện trong đồ án được thực hiện thông qua script:
...
Sau khi chỉnh các tham số trong `train.sh`, chạy:

```bash
cd scripts/phase_3_training/tiny_vla

bash train.sh
```

Các tham số chính thường cần chỉnh trong `train.sh`:

| Biến | Ý nghĩa |
|------|----------|
| `MODEL_BASE` | Đường dẫn tới backbone Llava-Pythia-400M |
| `OUTPUT_DIR` | Thư mục lưu checkpoint |
| `TASK` | Task Tiny-VLA sử dụng |
| `DATASET` | Dataset HDF5 dùng để train |
| `MAX_STEPS` | Tổng số bước huấn luyện |
| `SAVE_STEPS` | Chu kỳ lưu checkpoint |

---

## Kết quả sau khi train

Sau khi huấn luyện thành công, thư mục checkpoint sẽ chứa các file như:
...
Có thể kiểm tra nhanh dataset bằng các script đã có trong repository:

```bash
python scripts/phase_4_evaluation/tiny_vla/inspect_hdf5_dataset_summary.py
```

hoặc

```bash
python scripts/phase_4_evaluation/tiny_vla/inspect_one_hdf5_episode_report_simple.py
```

Điều này giúp xác nhận số episode, cấu trúc HDF5 và các trường dữ liệu trước khi bắt đầu huấn luyện.
### 12.4. Khai báo task trong TinyVLA gốc
